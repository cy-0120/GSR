...
const recentReportsStmt = db.prepare(`
  SELECT * FROM reports
  WHERE device_id = @deviceId AND created_at >= datetime('now', @sinceOffset)
`);
 ...
 */
function checkCooldown(deviceId, lat, lng) {
  const sinceOffset = `-${Math.ceil(ABUSE_COOLDOWN_MS / 1000)} seconds`;
  const recent = recentReportsStmt.all({ deviceId, sinceOffset });
